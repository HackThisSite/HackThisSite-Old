...
static int sql_Exec(char **argv, sqlite3 *db, char *sql, ...)
{
   ...
   if(ret) // Error, Busy, Permission, etc.
   {
      ...
      xbuf_xcat(get_reply(argv), 
           "<br><b style=\"color:#f00\">%s<\b><br>", *err?err:"error");
      if(err)
         sqlite3_free(err);
   }
   sqlite3_free(req);
...
int main(int argc, char **argv)
{
   ...
   // -------------------------------------------------------------------------
   {
      ...
      {
         "CREATE TABLE toons (id        integer primary key autoincrement,"
   ...
   // -------------------------------------------------------------------------
   xbuf_cat(reply, 
            "<br><h2>SELECT id,rate,name FROM toons (Custom Format):</h2>");
   static char szcr[] = "<br>", szseparator[] = ", ";
   sqlite3_stmt *stmt;
   ret = sql_QuerySteps(argv, db, &stmt, "SELECT id,rate,name FROM toons;", 0);
   if(!ret) // no error
   {
      do 
   ...
   // -------------------------------------------------------------------------
   xbuf_cat(reply, 
            "<br><h2>SELECT id,rate,name FROM toons (TEXT format):</h2><pre>");
   sql_Query(argv, db, reply, &fmt_text, "SELECT id,rate,name FROM toons;", 0);
   xbuf_cat(reply, "</pre>");

   xbuf_cat(reply, 
            "<br><h2>SELECT id,rate,name FROM toons (HTML format):</h2>");
   sql_Query(argv, db, reply, &fmt_html, "SELECT id,rate,name FROM toons;", 0);

   xbuf_cat(reply, 
            "<br><h2>SELECT id,rate,name FROM toons (JSON format):</h2><pre>");
   sql_Query(argv, db, reply, &fmt_json, "SELECT id,rate,name FROM toons;", 0);
   xbuf_cat(reply, "</pre>");

   xbuf_cat(reply, 
            "<br><h2>SELECT id,rate,name FROM toons (XML format):</h2><pre>");
   sql_Query(argv, db, reply, &fmt_xml, "SELECT id,rate,name FROM toons;", 0);
   xbuf_cat(reply, "</pre>");
   ...
   xbuf_cat(reply, 
            "<br><h2>SELECT id,rate,name FROM toons WHERE name LIKE 'T%'"
            " (PARSING RECORD):</h2>");
   u32 rate = 0;
   char name[40] = "T%";
   xbuf_t buf;
...
//           "SELECT id,rate,name FROM toons WHERE name = 'Tom';", 0);
             "SELECT id,rate,name FROM toons WHERE name LIKE ?;", "%s", name);
   ...
   *name = 0; // clear name
   ...
   // -------------------------------------------------------------------------
   xbuf_cat(reply, 
            "<br><h2>UPDATE toons SET name='Jerry Khan' WHERE id=2:</h2>");
   sql_Exec(argv, db, "UPDATE toons SET name='Jerry Khan' WHERE id=%u;", 2);
   sql_Query(argv, db, reply, &fmt_html, "SELECT id,rate,name FROM toons;", 0);
   ...
   // -------------------------------------------------------------------------
   xbuf_cat(reply, 
            "<br><h2>SELECT id,rate,name FROM toons WHERE rate > 4"
            " (PARSING TABLE):</h2>");
   rate = 4;
   ret = sql_QuerySteps(argv, db, &stmt, //"SELECT id,rate,name FROM toons;", 0);
         "SELECT id,rate,name FROM toons WHERE rate > ?;", "%u", rate);
   if(!ret) // no error
   {
      do 
      {  // format each line of data we fetch from the database
